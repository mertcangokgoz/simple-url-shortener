...
if (!empty($_GET['url'])) {
	...
	echo "<script>function go() {window.frames[0].document.body.innerHTML = '<form target=\"_parent\" method=\"get\" action=\"".$redirect."\"></form>';window.frames[0].document.forms[0].submit()}</script><iframe onload=\"window.setTimeout('go()', 99)\" src=\"about:blank\" style=\"visibility:hidden\"></iframe>";
